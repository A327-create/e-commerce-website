...
const NewArrivals = ({ setPage, setSelectedProductId }) => {
  ...
  useEffect(() => {
    fetchProducts()
      .then(data => {
        // Latest products — reverse order (newest last se first)
  ...
  return (
    <div className="container py-6">
      ...
      <div className="flex items-center gap-2 text-[#8B96A5] text-sm mb-6">
        ...
        <span className="text-[#1C1C1C] font-medium">New Arrivals</span>
      </div>

      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1C1C1C]">✨ New Arrivals</h1>
          <p className="text-[#8B96A5] text-sm mt-1">Latest {products.length} products</p>
        </div>
      </div>
            ...
            >
              ...
              <div className="w-full">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-base font-bold text-[#1C1C1C]">${product.price}</span>
                  {product.old_price && <span className="text-xs text-[#8B96A5] line-through">${product.old_price}</span>}
                </div>
              </div>
            </div>
          ))}
